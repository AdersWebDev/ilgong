...
function getDetailUrlFrom(producer, id) {
    const base = '/aders/dashboard/building';
    if ((producer || '').toLowerCase() === 'il') {
        return `${base}/detail-il.html?id=${encodeURIComponent(id)}`;
    }
    return `${base}/detail.html?producer=${encodeURIComponent(
        producer
