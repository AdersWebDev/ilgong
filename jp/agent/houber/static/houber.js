...
function collectFormData() {
    ...
    if (walkingTimeSelect) {
        ...
        if (walkingTimeLi && walkingTimeLi.dataset.value) {
            ...
            if (walkingTimeText && walkingTimeText !== '制限なし') {
...
function prepareBackendParams(formData) {
    ...
    if (formData.visaType && formData.visaType !== 'ビザを選択してください') {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (submitBtn) {
                ...
                submitBtn.textContent = '入力完了';
            }
            
            if (response.status === 400) {
                alert('メールアドレスへの送信に失敗しました。 住所を再度ご確認ください');
            } else if (response.status === 404) {
                alert('現在の条件に合う建物が見つかりません。');
            }
             else {
                alert('動作中にエラーが発生しました。 もう一度お試しください。');
            }
            return;
        }
        ...
        alert('リクエスト送信中にエラーが発生しました。 ネットワーク接続をご確認ください。');
    }
}
...
function getStatusMessageByFlow(flow, count) {
    const flowMessages = {
        'start': '検索リクエストを送信しました！ 条件に合った10棟の建物をお送りいたします',
        5: 'プロセス 申請待ち',
        10: 'プロセス開始中···',
        20: 'ご要望の分析中です···',
        30: 'データベースの集約中です···',
        ...
        70: 'きれいに包装中です',
        80: '建物{}個を見つけました',
        100: '建物{}個を見つけました、<br> 到着までに多少の時間がかかる場合がございます',
        '-1': 'エラー発生 リクエストを強制終了します'
    };
    
    let message = flowMessages[flow] || flowMessages[flow.toString()] || '動作中···';
...
function updateStatusModal(statusData) {
    ...
    const message = statusData.message || '動作中···';
    ...
    if (statusData.status === 'completed' || statusData.flow === 100) {
        ...
        if (statusData.details) {
            ...
            statusDetails.textContent = 'メールをご確認ください。';
        }
        statusDetails.style.display = 'block';
        ...
        statusDetails.textContent = statusData.details || 'もう一度お試しください。';
        statusDetails.style.display = 'block';
        modalFooter.style.display = 'flex';
...
function validateForm() {
    ...
    if (!emailRegex.test(email)) {
        showFieldError(emailInput, '正しいメール形式を入力してください。');
        return false;
    }

    if (!propertyLocation || propertyLocation === '') {
        showFieldError(propertyLocationSelect, 'ビザの種類をお選びください。');
        return false;
    }
    ...
    if (!privacyAgreement.checked) {
        showFieldError(privacyAgreement, '個人情報の収集および利用に同意するにチェックをお願いいたします。');
        return false;
    }
    ...
    if (emailField) {
        emailField.addEventListener('input', function() {
            if (this.value && !this.value.includes('@')) {
                this.setCustomValidity('正しいメール形式を入力してください。');
            } else {
